...
const gameKeydownHandler = (e) => {
  ...
  const currentCombat = state.currentCombat.value;
  ...
  if (e.code == "KeyW" || e.code == "ArrowUp") {
    ...
    const playerNextLocation = nextLocation(
      playerLocation,
      reverseDirection(playerDirection),
    );
    if (!canStepOn(dungeon, playerNextLocation)) {
      return;
    }
    ...
    const playerNextLocation = nextLocation(
      playerLocation,
      rightDirection(playerDirection),
    );
    if (!canStepOn(dungeon, playerNextLocation)) {
      return;
    }
